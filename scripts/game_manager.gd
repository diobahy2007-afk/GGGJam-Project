...
@onready var player = $Player  # Sesuaikan path ini dengan struktur scene Anda
...
func start_stand_off():
	...
	if camera and camera.has_method("enter_standoff"):
		print("→ Calling camera.enter_standoff()")
...
func _on_result_timer_timeout():
	...
	if current_enemy_index < enemies.size():
		...
		# Semua enemy kalah - stage complete!
		print("STAGE COMPLETE! All enemies defeated!")
		stage_complete()
